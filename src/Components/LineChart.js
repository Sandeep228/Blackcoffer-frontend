...
const LineChart = () => {
  ...
  async function getallData() {
    ...
    if (response.msg == "success") {
      let { resp } = response;
      let energyArr = response.resp.data;
      let arr = [];
      let arr2 = [];
      let arr3 = [];
      for (let i = 0; i < 50; i++) {
        arr[i] = energyArr[i].intensity;
        arr2[i] = parseInt(energyArr[i].likelihood);
        arr3[i] = parseInt(energyArr[i].relevance);
      }
  ...
  async function getAllYear(props) {
    let response = await getYear();
    let EnergyArr = response.data.data;
    const filteredData = EnergyArr.filter((item) => item.end_year == props);
    let arr = [];
    let arr2 = [];
    let arr3 = [];
    for (let i = 0; i < 50; i++) {
      arr[i] = filteredData[i]?.intensity;
      arr2[i] = filteredData[i]?.likelihood;
      arr3[i] = filteredData[i]?.relevance;
    }
    setGraphData({
      intensity: arr,
      likelihood: arr2,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <InputLabel id="dropdown-label">Year</InputLabel>
          <Select
            labelId="dropdown-label"
            id="dropdown"
            value={sector}
